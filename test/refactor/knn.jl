...
@testset "knn" begin
    ...
    region = BooleanInference.k_neighboring(tn, ws, doms, 1; k=2, max_tensors=10)
    @show region
end
...
@testset "original_test_for_neighboring" begin
    ...
    region = k_neighboring(problem.static, problem.ws, problem.doms, 1; max_tensors=100, k=1)
    @test region.tensors == [1, 3]
    @test Set(union(region.inner_vars, region.boundary_vars)) == Set([1, 4])

    region = k_neighboring(problem.static, problem.ws, problem.doms, 2; max_tensors=100, k=1)
...
@testset "original_test_for_k_neighboring" begin
    ...
    cnf = ∧(∨(b), ∨(a,¬c), ∨(d,¬b), ∨(¬c,¬d), ∨(a,e), ∨(a,e,¬c))
    problem = setup_from_cnf(cnf)
    region = k_neighboring(problem.static, problem.ws, problem.doms, 1; max_tensors=100, k=2)
    @test region.tensors == [1, 3, 4]
    @test Set(union(region.inner_vars, region.boundary_vars)) == Set([1, 3, 4])
    @test region.boundary_vars == [3]

    region = k_neighboring(problem.static, problem.ws, problem.doms, 2; max_tensors=100, k=2)
