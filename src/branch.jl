...
function OptimalBranchingCore.apply_branch(
    ...
    n_fixed = 0
    changed_vars = Int[]  # Track variables whose domains change
    ...
    for i in eachindex(propagated_doms)
        if propagated_doms[i].bits != new_doms[i].bits && i ∉ changed_vars
            push!(changed_vars, i)
        end
    end
    ...
    if any(dm -> dm.bits == 0x00, propagated_doms)
        ...
        return (TNProblem(problem.static, fill(DomainMask(0x00), length(propagated_doms)), 0, problem.ws), 0, changed_vars)
    end
    ...
    return (new_problem, 1, changed_vars)  # local_value = 1 (no scoring for now)
end
